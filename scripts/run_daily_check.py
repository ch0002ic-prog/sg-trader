...
def main() -> int:
    ...
    parser.add_argument("--enforce-v11-6", action="store_true", help="Switch active profile to v11_6_tuned before running checks")
    parser.add_argument("--enforce-v11-7", action="store_true", help="Switch active profile to v11_7_tuned before running checks")
    args = parser.parse_args()

    selected = int(args.enforce_v8) + int(args.enforce_v10) + int(args.enforce_v11_2) + int(args.enforce_v11_3) + int(args.enforce_v11_4) + int(args.enforce_v11_5) + int(args.enforce_v11_6) + int(args.enforce_v11_7)
    if selected > 1:
        raise ValueError("Use only one of --enforce-v8, --enforce-v10, --enforce-v11-2, --enforce-v11-3, --enforce-v11-4, --enforce-v11-5, --enforce-v11-6, or --enforce-v11-7")
    ...
    if args.enforce_v11_6:
        switch_script = REPORTS / "v11_6_switch_to_tuned.sh"
        if not switch_script.exists():
            raise FileNotFoundError(f"Missing switch script: {switch_script}")
        run(["bash", str(switch_script)])

    if args.enforce_v11_7:
        switch_script = REPORTS / "v11_7_switch_to_tuned.sh"
        if not switch_script.exists():
            raise FileNotFoundError(f"Missing switch script: {switch_script}")
        run(["bash", str(switch_script)])
