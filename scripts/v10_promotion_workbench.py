...
def main() -> int:
    parser = argparse.ArgumentParser(description="V10 promotion workbench (hard DD envelope vs active v9)")
    parser.add_argument("--base-profile", default="reports/v9_tuned_profile.json")
    parser.add_argument("--base-overrides", default="reports/v9_tuned_overrides.csv")
    parser.add_argument("--rerisk-grid", default="0.04,0.06,0.08")
    parser.add_argument("--hold-grid", default="0.42,0.44,0.46")
    parser.add_argument("--cap-grid", default="0.03")
    parser.add_argument("--derisk-grid", default="0.14,0.16,0.18")
    ...
    parser.add_argument("--out-prefix", default="reports/v10_promotion_workbench")
    ...
    if not folds:
        raise RuntimeError("Could not construct OOT folds for v10 workbench")
    ...
    for rerisk, hold, cap, derisk, scale in itertools.product(rerisk_grid, hold_grid, cap_grid, derisk_grid, scale_grid):
        ...
        cand_profile["profile"] = "v10_candidate"
    ...
    df = df.sort_values(
        ["min_median_delta_terminal", "min_pass_rate_terminal", "worst_median_delta_maxdd", "worst_fold_maxdd", "full_delta_terminal"],
    ...
    if not passing.empty:
        ...
        decision = "PROMOTE_V10"

        v10_profile = dict(base_profile)
        v10_profile["profile"] = "v10_tuned"
        v10_profile["rerisk"] = float(selected["rerisk"])
        v10_profile["hold"] = float(selected["hold"])
        v10_profile["cap"] = float(selected["cap"])
        v10_profile["derisk"] = float(selected["derisk"])
        ...
        Path(str(out_prefix) + "_promote_profile.json").write_text(json.dumps(v10_profile, indent=2) + "\n", encoding="utf-8")
        pd.DataFrame([{"state": k, "override_exposure": v} for k, v in sorted(v10_overrides.items())]).to_csv(
    ...
    lines = [
        ...
        "",
        f"- base_profile: {base_profile.get('profile', 'v9_tuned')}",
        f"- folds: {len(folds)}",
