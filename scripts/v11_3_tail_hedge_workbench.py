...
def simulate_returns_with_cost_and_hedge(
    ...
    for _, df in td.items():
        ...
        stress1_mask = (w["date"] >= stress1_start) & (w["date"] < stress1_end)
        stress2_mask = (w["date"] >= stress2_start) & (w["date"] < stress2_end)
        stress_mask = stress1_mask | stress2_mask
    ...
    for cost in cost_grid:
        ...
        for _, train_end, val_end in folds:
            ret = simulate_returns_with_cost_and_hedge(
                ...
                stress1_start, stress1_end, stress2_start, stress2_end,
                stress_cap, state_substr, state_cap,
        ...
        stress1_ret = simulate_returns_with_cost_and_hedge(
            ...
            stress1_start, stress1_end, stress2_start, stress2_end,
            stress_cap, state_substr, state_cap,
            start=stress1_start, end=stress1_end,
        )
        ...
        stress2_ret = simulate_returns_with_cost_and_hedge(
            ...
            stress1_start, stress1_end, stress2_start, stress2_end,
            stress_cap, state_substr, state_cap,
            start=stress2_start, end=stress2_end,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V11.3 tail-hedged workbench")
    parser.add_argument("--base-profile", default="reports/v11_2_tuned_profile.json")
    parser.add_argument("--base-overrides", default="reports/v11_2_tuned_overrides.csv")
    parser.add_argument("--stress-cap-grid", default="0.10,0.12,0.14,0.16,0.18,0.20,0.24,0.30")
    parser.add_argument("--state-cap-grid", default="0.10,0.12,0.14,0.16,0.18,0.20,0.24,0.30")
    parser.add_argument("--scale-grid", default="0.90,1.00,1.05,1.10")
    ...
    parser.add_argument("--out-prefix", default="reports/v11_3_tail_hedge_run1")
    ...
    lines = [
        ...
        f"- stress_windows: [{args.stress1_start}, {args.stress1_end}) and [{args.stress2_start}, {args.stress2_end})",
        "",
        "## Top 20",
