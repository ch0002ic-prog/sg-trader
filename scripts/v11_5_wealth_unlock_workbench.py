...
def apply_state_specific_scaling(
    ...
    for state, value in base_overrides.items():
        scale = overall_scale
        if derisk_substr and derisk_substr in state:
            scale *= derisk_scale
        if panic_substr and panic_substr in state:
            scale *= panic_scale
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V11.5 wealth unlock workbench")
    ...
    parser.add_argument("--stress-cap-grid", default="0.14,0.16,0.18,0.20,0.22")
    parser.add_argument("--derisk-cap-grid", default="0.24,0.28,0.30,0.34")
    parser.add_argument("--panic-cap-grid", default="0.08,0.10,0.12,0.14,0.16,0.20")
    parser.add_argument("--overall-scale-grid", default="0.95,1.00,1.05,1.10")
    parser.add_argument("--derisk-scale-grid", default="0.90,1.00,1.10")
    parser.add_argument("--panic-scale-grid", default="0.85,1.00,1.15")
    ...
    parser.add_argument("--min-margin-floor", type=float, default=0.002)
    parser.add_argument("--out-prefix", default="reports/v11_5_wealth_unlock_run1")
    ...
    rows = []
    for stress_cap, derisk_cap, panic_cap, overall_scale, derisk_scale, panic_scale in itertools.product(
        stress_cap_grid,
        derisk_cap_grid,
        panic_cap_grid,
        overall_scale_grid,
