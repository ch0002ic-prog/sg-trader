...
def build_candidate_payloads(
    ...
    for _, df in td.items():
        ...
        ret = pd.to_numeric(df["ret"], errors="coerce").fillna(0.0).to_numpy(dtype=float)
        default_exp = pd.to_numeric(df["default_exp"], errors="coerce").fillna(0.0).to_numpy(dtype=float)

        if overrides:
            override_series = pd.Series(overrides)
            mapped = pd.Series(states).map(override_series).to_numpy(dtype=float)
            exp = np.where(np.isnan(mapped), default_exp, mapped)
        else:
            exp = default_exp.copy()

        if derisk_substr:
            derisk_mask = pd.Series(states).str.contains(derisk_substr, regex=False, na=False).to_numpy()
            exp[derisk_mask] = np.minimum(exp[derisk_mask], float(derisk_cap))

        if panic_substr:
            panic_mask = pd.Series(states).str.contains(panic_substr, regex=False, na=False).to_numpy()
            exp[panic_mask] = np.minimum(exp[panic_mask], float(panic_cap))

        payloads.append({"date": dates, "ret": ret, "exp_no_stress": exp})
    return payloads
...
def simulate_returns_with_cost_and_hedge(
    ...
    for item in payloads:
        ...
        stress_mask = ((d >= np.datetime64(stress1_start)) & (d < np.datetime64(stress1_end))) | ((d >= np.datetime64(stress2_start)) & (d < np.datetime64(stress2_end)))
        e[stress_mask] = np.minimum(e[stress_mask], float(stress_cap))

        exp_lag = np.empty_like(e)
        exp_lag[0] = float(hold_fill)
        exp_lag[1:] = e[:-1]

        gross = r * exp_lag
        turnover = np.empty_like(exp_lag)
        turnover[0] = 0.0
        turnover[1:] = np.abs(exp_lag[1:] - exp_lag[:-1])
        net = gross - turnover * cost
        parts.append(pd.DataFrame({"date": d, "strategy_ret": net}))
...
def evaluate_candidate(
    ...
    panic_substr: str,
    panic_cap: float,
    fold_maxdd_cap: float,
    stress_maxdd_cap: float,
    full_maxdd_cap: float,
) -> dict:
    ...
    for cost in cost_grid:
        ...
        if (
            worst_fold_maxdd < float(fold_maxdd_cap)
            or worst_stress_maxdd < float(stress_maxdd_cap)
            or full_maxdd < float(full_maxdd_cap)
        ):
            infeasible = True
            break
    ...
    if infeasible:
        return {
            "min_median_terminal": -1e9,
            "min_pass_rate_terminal": 0.0,
            "avg_full_terminal": -1e9,
            "avg_full_sharpe": -1e9,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V11.7 local wealth refinement workbench")
    parser.add_argument("--base-profile", default="reports/v11_6_local_wealth_refine_run1_promote_profile.json")
    parser.add_argument("--base-overrides", default="reports/v11_6_local_wealth_refine_run1_promote_overrides.csv")
    parser.add_argument("--stress-cap-grid", default="0.15,0.16,0.17")
    parser.add_argument("--derisk-cap-grid", default="0.32,0.34,0.36")
    parser.add_argument("--panic-cap-grid", default="0.07,0.08,0.09")
    parser.add_argument("--overall-scale-grid", default="0.99,1.00,1.01")
    parser.add_argument("--risk-on-scale-grid", default="1.00,1.02,1.04,1.06")
    parser.add_argument("--derisk-scale-grid", default="0.99,1.00,1.01")
    parser.add_argument("--panic-scale-grid", default="0.83,0.85,0.87")
    ...
    parser.add_argument("--out-prefix", default="reports/v11_7_local_wealth_refine_run1")
    ...
    for stress_cap, derisk_cap, panic_cap, overall_scale, risk_on_scale, derisk_scale, panic_scale in itertools.product(
        ...
        m = evaluate_candidate(
            ...
            panic_substr=str(args.panic_substr),
            panic_cap=float(panic_cap),
            fold_maxdd_cap=float(args.fold_maxdd_cap),
            stress_maxdd_cap=float(args.stress_maxdd_cap),
            full_maxdd_cap=float(args.full_maxdd_cap),
        )
