...
def evaluate_candidate(
    ...
    by_cost_df = pd.DataFrame(by_cost)

    cand_full = perf_from_returns(simulate_returns_with_cost(cand_td, cand_overrides, float(cand_profile["hold"]), min(cost_grid)))
    base_full = perf_from_returns(simulate_returns_with_cost(base_td, base_overrides, float(base_profile["hold"]), min(cost_grid)))

    return {
        "min_median_delta_terminal": float(by_cost_df["median_delta_terminal"].min()),
        "min_pass_rate_terminal": float(by_cost_df["pass_rate_terminal"].min()),
        "worst_median_delta_maxdd": float(by_cost_df["median_delta_maxdd"].min()),
        "avg_median_delta_terminal": float(by_cost_df["median_delta_terminal"].mean()),
        "avg_median_delta_sharpe": float(by_cost_df["median_delta_sharpe"].mean()),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V8 promotion workbench (robust vs active v7)")
    parser.add_argument("--base-profile", default="reports/v7_tuned_profile.json")
    parser.add_argument("--base-overrides", default="reports/v7_tuned_overrides.csv")
    parser.add_argument("--rerisk-grid", default="0.08,0.1,0.12")
    parser.add_argument("--hold-grid", default="0.42,0.45,0.48")
    parser.add_argument("--cap-grid", default="0.03,0.05")
    parser.add_argument("--derisk-grid", default="0.10,0.12,0.14")
    parser.add_argument("--scale-grid", default="1.1,1.2,1.25,1.3")
    ...
    parser.add_argument("--dd-tol", type=float, default=-0.02)
    parser.add_argument("--out-prefix", default="reports/v8_promotion_workbench")
    ...
    folds = build_folds(load_td(base_profile, base_overrides_raw), n_folds=int(args.folds))
    if not folds:
        raise RuntimeError("Could not construct OOT folds for v8 workbench")
    ...
    for rerisk, hold, cap, derisk, scale in itertools.product(rerisk_grid, hold_grid, cap_grid, derisk_grid, scale_grid):
        ...
        cand_profile["profile"] = "v8_candidate"
        ...
        metrics = evaluate_candidate(cand_profile, cand_overrides, base_profile, base_overrides_raw, folds, cost_grid)
        rows.append(
            {
                "rerisk": rerisk,
                "hold": hold,
    ...
    df = df.sort_values(
        ["min_median_delta_terminal", "min_pass_rate_terminal", "worst_median_delta_maxdd", "full_delta_terminal"],
    ...
    if not passing.empty:
        ...
        v8_profile = dict(base_profile)
        v8_profile["profile"] = "v8_tuned"
        v8_profile["rerisk"] = float(selected["rerisk"])
        v8_profile["hold"] = float(selected["hold"])
        v8_profile["cap"] = float(selected["cap"])
        v8_profile["derisk"] = float(selected["derisk"])
        ...
        Path(str(out_prefix) + "_promote_profile.json").write_text(json.dumps(v8_profile, indent=2) + "\n", encoding="utf-8")
        pd.DataFrame([{"state": k, "override_exposure": v} for k, v in sorted(v8_overrides.items())]).to_csv(
    ...
    lines = [
        ...
        "",
        f"- base_profile: {base_profile.get('profile', 'v7_tuned')}",
        f"- folds: {len(folds)}",
