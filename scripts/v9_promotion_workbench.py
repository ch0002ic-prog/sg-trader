...
def evaluate_candidate(
    ...
    # full-sample at base persistence + lowest cost
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V9 promotion workbench (scenario-robust vs active v8)")
    parser.add_argument("--base-profile", default="reports/v8_tuned_profile.json")
    parser.add_argument("--base-overrides", default="reports/v8_tuned_overrides.csv")
    parser.add_argument("--rerisk-grid", default="0.06,0.08,0.1")
    parser.add_argument("--hold-grid", default="0.46,0.48,0.5")
    parser.add_argument("--cap-grid", default="0.03")
    parser.add_argument("--derisk-grid", default="0.12,0.14,0.16")
    parser.add_argument("--scale-grid", default="1.2,1.25,1.3")
    ...
    parser.add_argument("--out-prefix", default="reports/v9_promotion_workbench")
    ...
    if not folds:
        raise RuntimeError("Could not construct OOT folds for v9 workbench")
    ...
    for rerisk, hold, cap, derisk, scale in itertools.product(rerisk_grid, hold_grid, cap_grid, derisk_grid, scale_grid):
        ...
        cand_profile["profile"] = "v9_candidate"
    ...
    if not passing.empty:
        ...
        v9_profile = dict(base_profile)
        v9_profile["profile"] = "v9_tuned"
        v9_profile["rerisk"] = float(selected["rerisk"])
        v9_profile["hold"] = float(selected["hold"])
        v9_profile["cap"] = float(selected["cap"])
        v9_profile["derisk"] = float(selected["derisk"])
        ...
        Path(str(out_prefix) + "_promote_profile.json").write_text(json.dumps(v9_profile, indent=2) + "\n", encoding="utf-8")
        pd.DataFrame([{"state": k, "override_exposure": v} for k, v in sorted(v9_overrides.items())]).to_csv(
    ...
    lines = [
        ...
        "",
        f"- base_profile: {base_profile.get('profile', 'v8_tuned')}",
        f"- folds: {len(folds)}",
